...
object XLSFlatFileReader extends ExcelFlatFileReader {
  def makeLineReader(in: InputStream, options: FlatFileReaderOptions): LazySeq[Try[FlatFileParsedRow]] = {
    ...
    if (!ExcelFlatFileReader.isXLSFormat(bis)) throw ExcelFlatFileReaderException.InvalidExcelFile("Not an XLS File")
    new XLSStreamReaderImpl(bis, options)
  }
}