...
object ExcelFlatFileReader extends ExcelFlatFileReader {
  ...
  private val excelDateParser: DateTimeFormatter = {
    val parsers = Array(
        "M/d/yyyy",        // 3/14/2001 => 1/1/2013
        "M-d-yyyy",        // 3-14-2001 => 1-1-2013
        "E, MMM dd, yyyy", // Wednesday, March 14, 2001 => Tuesday, January 01, 2013
   ...
   */
  def parseExcelDate(dateStr: String): LocalDate = try {
    require(dateStr.isNotBlank, "dateStr is blank!")
    val date: LocalDate = excelDateParser.parseDateTime(dateStr).toLocalDate()
    ...
    case ex: UnsupportedOperationException => throw new ExcelFlatFileReaderException.InvalidDateFormat(s"Invalid Date Format encountered: $dateStr") // - if parsing is not supported
    case ex: IllegalArgumentException => throw new ExcelFlatFileReaderException.InvalidDateFormat(s"Invalid Date Format encountered: $dateStr") // - if the text to parse is invalid
  }
